  ...
  if (!isOpen) {
    return (
      <button 
        ...
        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-indigo-600 hover:text-indigo-700 hover:bg-indigo-50 transition-all text-xs font-medium border border-indigo-400"
      >
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/20 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white border border-gray-200 w-full max-w-md rounded-[2rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
        ...
        <div className="p-8 pt-0 space-y-6">
          ...
          <button 
            onClick={() => setIsOpen(false)}
            className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3.5 rounded-xl transition-all shadow-lg shadow-indigo-600/20 active:scale-95 mt-4"
          >
            Mengerti, Lanjutkan!
          </button>
        </div>
...
function GuideStep({ icon, title, desc }: { icon: React.ReactNode, title: string, desc: string }) {
  return (
    <div className="flex gap-4">
      <div className="shrink-0 w-10 h-10 bg-gray-100 rounded-xl flex items-center justify-center text-gray-500">
        {icon}
      </div>
      <div>
